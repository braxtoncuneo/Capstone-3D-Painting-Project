		...
		SubShader
	{
		...
		Pass
		{
			...
			*/ 
			...
			/*
			** Calculates the ratio of the distance between the default fragement position to the 
			** camera and the distance between the ray's intersection with the brush cursor and
			** the camera
			**
			** worldCoord: the world-space coordinates of the default fragment position
			** viewCoord: the offset of the camera from the default fragment position (in object-space)
			**
			** Returns -1 if the ray never intersects with the brush tip
			*/ 
			float getBrushTime(in float4 worldCoord, in float3 viewCoord) {
				...
				if (length(bNear) < 1.0) {
					return  length(bFrag-bCam)/(bIntr-sqrt(1-dot(bNear,bNear)));
				}
				else {
					return -1;
				}
			...
			*/
			fOut frag (v2f i)
			{
				...
				// Calculates the depth of the trace's intersection with the brush tip
				float brushTime = getBrushTime(i.world, i.camera);
				float3 brushHit = i.world + fragCamOffset * brushTime;
				float brushZ = mul(UNITY_MATRIX_VP, float4(brushHit, 1.0)).z - 0.1;

				
				result.color = float4(0, 0, 0, 0);

				// If the brush tip is intersected and is intersected sooner
				// than any voxels, its color and depth are used
				if (brushZ < rayZ && brushTime != -1 ) {
					result.depth = brushZ;
					result.color = float4(brush_color);
				}
				else {
					result.depth = rayZ;
				}
